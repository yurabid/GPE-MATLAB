...
mu_last = mu;
%%
niter_inner = 200; %number of internal iterations
...
for j=start+1:niter_outer
    ...
    for i=1:niter_inner
        ...
        if(omega ~= 0)
            lphi = tmp;
            for ii = 1:n_cn
                %lphi = tmp + dt*omega*0.5/h*(X.*(circshift(tmp,1)-circshift(tmp,-1)) - Y.*(circshift(tmp,[0 1])-circshift(tmp,[0 -1])));
                lphi = tmp + dt*1i*omega/(12*h)*(Xg.*(-circshift(lphi,2)+8*circshift(lphi,1)-8*circshift(lphi,-1)+circshift(lphi,-2)) - ...
