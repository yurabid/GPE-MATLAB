...
%     phitm(i) = phitm(i-1) + tstep*(invC*(zztmp-(zz0(i)+zz0(i-1))/2) + IC*zztmp/sqrt(1-zztmp^2)*cos(phitmp) + Jb*(2*zztmp^2-1)/sqrt(1-zztmp^2)*cos(phitmp));
    dmu = (edrive(i-1) + lam*zztm(i-1) + IC(i-1)*zztm(i-1)/abs(sqrt(1-zztm(i-1)^2))*cos(phitm(i-1)) );
    zztmp = zztm(i-1) - tstep/2*( IC(i-1)*abs(sqrt(1-zztm(i-1)^2))*sin(phitm(i-1)) + G*dmu);
    phitmp = phitm(i-1) + tstep/2*dmu;
    dmu = ((edrive(i)+edrive(i-1))/2 + lam*zztmp + (IC(i-1)+IC(i))/2*zztmp/abs(sqrt(1-zztmp^2))*cos(phitmp) );
    zztm(i) = zztm(i-1) - tstep*( (IC(i-1)+IC(i))/2*abs(sqrt(1-zztmp^2))*sin(phitmp)  + G*dmu) ;
