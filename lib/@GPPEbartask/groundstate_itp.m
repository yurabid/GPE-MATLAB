...
tmp2 = real(phi.*conj(phi)).*g+task.Fi(1:nx,1:nx,1:nx)+task.bar_Fi(1:nx,1:nx,1:nx)+V;
while true
    i=i+1;
...
%         task.set_pot_bc_bar(bdens);
        [task.Fi,~]=task.V_cycle(task.Fi,tmp,h,sz(1));
%         [task.bar_Fi,~]=task.V_cycle(task.bar_Fi,bdens,h,sz(1));
    end
    tmp2 = tmp.*g+task.Fi(1:nx,1:nx,1:nx)+task.bar_Fi(1:nx,1:nx,1:nx)+V;
    task.current_state = phi;
% subplot(1,2,1)
% imagesc(abs(phi(:,:,64)));
% subplot(1,2,2)
% imagesc(real(task.Fi(:,:,64)));
% subplot(2,2,3)
% plot(MU);
% subplot(2,2,4)
