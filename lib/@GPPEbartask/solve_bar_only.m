function phi = solve_bar_only(task,ddt,niter_inner,niter_outer)
...
%         [task.Fi,~]=task.V_cycle(task.Fi,tmp2,h,sz(1)); 
        ...
        task.set_pot_bar_an(rb);

%         VV = g.*tmp2+task.getVtotal(time2)-mu+task.Fi(1:nx,1:nx,1:nx)+task.bar_Fi(1:nx,1:nx,1:nx);        
%         task.bar_pot = interpn(grid.mesh.y,grid.mesh.x,grid.mesh.z,task.Fi(1:nx,1:nx,1:nx),rb(2),rb(1),rb(3));
        task.bar_pot = interpn(grid.mesh.y,grid.mesh.x,grid.mesh.z,task.Fi(1:nx,1:nx,1:nx),rb(2),rb(1),rb(3));
...
%         Fz = -real(sum(tmp3(:).*grid.kz(:))/grid.nx^3);        
        ...
        mbar = pi^1.5*task.bar_rad^3*task.bar_amp;
        Fx = -grid.integrate(bdens.*Fx(1:nx,1:nx,1:nx)/h)/mbar;
        Fy = -grid.integrate(bdens.*Fy(1:nx,1:nx,1:nx)/h)/mbar;
        Fz = -grid.integrate(bdens.*Fz(1:nx,1:nx,1:nx)/h)/mbar;
