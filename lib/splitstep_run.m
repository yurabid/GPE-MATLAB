...
tmp2 = abs(tmp.*conj(tmp));
    LLL = -(imag(sum(sum(sum(conj(phi).*(X.*(-circshift(phi,2)+8*circshift(phi,1)-8*circshift(phi,-1)+circshift(phi,-2)) -...
        Y.*(-circshift(phi,[0 2])+8*circshift(phi,[0 1])-8*circshift(phi,[0 -1])+circshift(phi,[0 -2]))))))))*h*hz./(12*NN0);
mu = (abs(sum(sum(sum(abs(conj(tmp).*ifftn(kk.*fftn(tmp))) + (V+g*tmp2).*tmp2))))*h*h*hz)./NN0 - omega*LLL;
%mu_last = mu;
%%
...
for j=start+1:niter_outer
    ...
    if(doCoreDetect > 0)
        ...
        coresp = coresp(2:end,:);
        coresm = coresm(2:end,:);
        fv = isosurface(X,Y,Z,abs(phi),0.2*maxx);
