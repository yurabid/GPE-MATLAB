function [s,v, varargout] = velocity_field2(slice,r,cphase,slice0)
...
%   - s: streamlines of the vector velocity field
	...
	vx = -imag(slice.*conj(sx) - conj(slice).*sx)./(abs(slice).^2).*(abs(slice)>1e-4)./(2*h);% - vx0;
	vy = -imag(slice.*conj(sy) - conj(slice).*sy)./(abs(slice).^2).*(abs(slice)>1e-4)./(2*h);% - vy0;
