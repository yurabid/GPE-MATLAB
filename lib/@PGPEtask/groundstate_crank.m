...
while delta > eps && i<20000

    lphi = phi;
    lphir = phir;
    for ii = 1:n_cn        
        lphi = phi - dt*(grid.etot.*lphi + grid.grid2spop((V+g*abs(lphir.^2)).*lphir));
        lphi = 0.5*(phi+lphi);
        lphir = grid.sp2grid(lphi);
    end
    phi = phi - dt*(grid.etot.*lphi + grid.grid2spop((V+g*abs(lphir.^2)).*lphir));
...
% 	tmp2 = real(phi.*conj(phi));
    ...
    MU(i) = mu;
    if(nargout >= 3)
        MU2(i) = real(sum(grid.to1d(grid.etot.*abs(phi).^2 + conj(phi).*(grid.grid2spop((V+g*abs(phir.^2)).*phir)))));
    end
    if(i>50)
